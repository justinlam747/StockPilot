...
RSpec.describe Reports::WeeklyGenerator do
  ...
  describe "#generate" do
    context "with full data" do
      ...
      let(:variant_a) { create(:variant, shop: shop, product: product_a, sku: "WA-001", title: "Small", price: 10.00, supplier: supplier) }
      let(:variant_b) { create(:variant, shop: shop, product: product_b, sku: "WB-001", title: "Large", price: 25.00, supplier: supplier) }
      ...
      before do
        ...
        create(:inventory_snapshot, shop: shop, variant: variant_a, available: 100, on_hand: 100, created_at: week_start + 2.hours)
        create(:inventory_snapshot, shop: shop, variant: variant_b, available: 50, on_hand: 50, created_at: week_start + 2.hours)
        create(:inventory_snapshot, shop: shop, variant: variant_c, available: 30, on_hand: 30, created_at: week_start + 2.hours)

        # End-of-week snapshots (Sunday)
        create(:inventory_snapshot, shop: shop, variant: variant_a, available: 60, on_hand: 60, created_at: week_end - 6.hours)
        create(:inventory_snapshot, shop: shop, variant: variant_b, available: 10, on_hand: 10, created_at: week_end - 6.hours)
        create(:inventory_snapshot, shop: shop, variant: variant_c, available: 25, on_hand: 25, created_at: week_end - 6.hours)
      ...
      describe "top_sellers" do
        ...
        it "limits to top 10 sellers" do
          ...
          12.times do |i|
            ...
            create(:inventory_snapshot, shop: shop, variant: v, available: 200, on_hand: 200, created_at: week_start + 2.hours)
            create(:inventory_snapshot, shop: shop, variant: v, available: 10, on_hand: 10, created_at: week_end - 6.hours)
          end
      ...
      describe "reorder_suggestions" do
        it "groups low-stock variants by supplier" do
          ...
          create(:inventory_snapshot, shop: shop, variant: variant_b, available: 3, on_hand: 3, created_at: Time.current)
        ...
        it "includes suggested_qty based on threshold calculation" do
          ...
          create(:inventory_snapshot, shop: shop, variant: variant_a, available: 3, on_hand: 3, created_at: Time.current)
        ...
        it "excludes variants without a supplier" do
          ...
          create(:inventory_snapshot, shop: shop, variant: variant_c, available: 2, on_hand: 2, created_at: Time.current)
    ...
    context "when variants have no sales (end quantity >= start quantity)" do
      it "excludes variants with zero or negative sales" do
        ...
        create(:inventory_snapshot, shop: shop, variant: variant, available: 10, on_hand: 10, created_at: week_start + 2.hours)
        create(:inventory_snapshot, shop: shop, variant: variant, available: 50, on_hand: 50, created_at: week_end - 6.hours)
