...
RSpec.describe Agents::InventoryMonitor do
  ...
  describe "tool methods" do
    ...
    let(:variant_low) { create(:variant, shop: shop, product: product, sku: "TW-LOW", title: "Small", price: 10.00, supplier: supplier) }
    let(:variant_oos) { create(:variant, shop: shop, product: product, sku: "TW-OOS", title: "Large", price: 20.00, supplier: supplier) }
    ...
    describe "#tool_draft_purchase_order" do
      it "creates a draft PO with line items for the supplier" do
        ...
        expect(po.purchase_order_line_items.size).to be >= 1
      end
      ...
      it "calculates correct suggested quantities" do
        ...
        if line_item
          ...
          expect(line_item.quantity_ordered).to eq(15)
        end
      end
